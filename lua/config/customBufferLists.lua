-- bufferlist.lua
-- Floating buffer list: Oldest at top, Newest at bottom.
...
vim.api.nvim_create_user_command("Buffer", function()
	...
	-- 1. Get currently listed buffers
	local current_listed = vim.fn.getbufinfo({ buflisted = 1 })
	local listed_dict = {}
	for _, b in ipairs(current_listed) do
		listed_dict[b.bufnr] = true
	end

	-- 2. Clean buffer_order: Remove anything no longer listed
	local clean_order = {}
	for _, bufnr in ipairs(buffer_order) do
		if listed_dict[bufnr] then
			table.insert(clean_order, bufnr)
		end
	end
	buffer_order = clean_order

	-- 3. Add NEW buffers to the END of buffer_order
	for _, b in ipairs(current_listed) do
		if not contains(buffer_order, b.bufnr) then
			table.insert(buffer_order, b.bufnr)
		end
	end
	...
	for i, bufnr in ipairs(buffer_order) do
		...
		-- Formatting the line with the index
